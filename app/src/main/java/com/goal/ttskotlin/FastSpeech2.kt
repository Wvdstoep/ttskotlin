...
class FastSpeech2(modulePath: String?) : AbstractModule() {
    ...
    fun getMelSpectrogram(inputIds: IntArray, speed: Float): TensorBuffer {
        ...
        mModule!!.runForMultipleInputsOutputs(
            arrayOf<Any>(inputs, Array(1) {
                IntArray(
                    1
                )
            }, intArrayOf(0), floatArrayOf(speed), floatArrayOf(1f), floatArrayOf(1f)),
            outputMap
        )
        ...
        val spectrogram: TensorBuffer = TensorBuffer.createFixedSize(shape, DataType.FLOAT32)
